...
const AddSize = () => {
    ...
    const mutatiton = useMutation({
        mutationFn: async (size) => {
            const { data } = await axios.post(`http://localhost:8080/api/size/add`, size)
            return data;
        },
        onSuccess: () => {
        ...
        <section className="main tw-p-5">
            <div className="container">
                <div className="tw-flex tw-justify-between tw-mb-6">
                    <h1 className="tw-text-3xl">Add Size</h1>
                    <Button className=""><Link to={`/admin/tags`}>Danh sách Size</Link></Button>
                </div>
