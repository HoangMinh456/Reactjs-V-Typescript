  ...
  (
    ...
    return (
      <div
        ref={ref}
        className={cn(
          "tw-grid tw-min-w-[8rem] tw-items-start tw-gap-1.5 tw-rounded-lg tw-border tw-border-slate-200 tw-border-slate-200/50 tw-bg-white tw-px-2.5 tw-py-1.5 tw-text-xs tw-shadow-xl dark:tw-border-slate-800 dark:tw-border-slate-800/50 dark:tw-bg-slate-950",
          className
        )}
      >
          ...
          {payload.map((item, index) => {
            ...
            return (
              <div
                key={item.dataKey}
                className={cn(
                  "tw-flex tw-w-full tw-flex-wrap tw-items-stretch tw-gap-2 [&>svg]:tw-h-2.5 [&>svg]:tw-w-2.5 [&>svg]:tw-text-slate-500 dark:[&>svg]:tw-text-slate-400",
                ...
                {formatter && item?.value !== undefined && item.name ? (
                  ...
                  <>
                    ...
                    >
                      ...
                      {item.value && (
                        <span className="tw-font-mono tw-font-medium tw-tabular-nums tw-text-slate-950 dark:tw-text-slate-50">
        ...
        {payload.map((item) => {
          ...
          return (
            <div
              key={item.value}
              className={cn(
                "tw-flex tw-items-center tw-gap-1.5 [&>svg]:tw-h-3 [&>svg]:tw-w-3 [&>svg]:tw-text-slate-500 dark:[&>svg]:tw-text-slate-400"
              )}
            >
